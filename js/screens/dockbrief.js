...
CC.DockBriefScreen = class {
  ...
  draw(ctx) {
    ...
    if (this.lv.types.length > lanes) CC.U.text(ctx, `${this.lv.types.length} types · ${lanes} lanes — last lane swaps type (buy lanes in Upgrade Bay)`, W / 2, 705, { size: 12, weight: 700, color: C.Warn });
    else if (this.lv.timerPressure) CC.U.text(ctx, 'forklift is on the clock — timer pressure', W / 2, 705, { size: 12, weight: 700, color: C.Warn });
