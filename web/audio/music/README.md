# Crate Crush (Dock Rush mode) — music / groove loops
...
`grooveIntensity(k)` (k 0..1, called ~4 Hz from `DockRun`) scales the loop's bed gain 0.08 → 0.28 when the sample is present; with the procedural bed it drives the same hats/bass/sparkle ladder. Kade: this is the hook for a stem-based groove (drop stems, read `k`).
